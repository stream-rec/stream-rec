...
> - `下载文件夹`:
    ...
    。那么，您应该将`/path/to/container/folder`作为输出文件夹。 Docker compose 请使用services.backend.volumes.target 作为输出文件夹。
...
> [!NOTE]\
> - 保存按钮仅附加到您当前正在编辑的平台。确保在完成编辑每个平台的设置后单击“保存设置”按钮。
...
> - 如果仍然有主播使用模板用户，模板用户将无法删除。也无法将其转换为普通主播。
