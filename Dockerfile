...
RUN ARCH=$(uname -m) && \
    if [ "$ARCH" = "x86_64" ]; then \
      URL="https://github.com/hua0512/rust-srec/releases/download/v0.3.1/strev-linux-amd64"; \
    elif [ "$ARCH" = "aarch64" ]; then \
      URL="https://github.com/hua0512/rust-srec/releases/download/v0.3.1/strev-linux-arm64"; \
    fi && \
    curl -L $URL -o strev && \
